# Frontend Deployment Guide – Vue (Vite) → Vercel
...
## 🔹 Step 1: Prepare the frontend project for Vercel
...
* If it finishes without errors → you’re good.
...
## 🔹 Step 2: Create a Vercel project from your repo
...
## 🔹 Step 3: Tell Vercel the **frontend folder**

Because your Vue app is not at the root but in `/frontend`:

1. Under **“Root Directory”** choose or enter:
   `frontend`
2. Click **“Continue”**.
...
## 🔹 Step 4: Build settings (Vite + Vue)
...
* **Output Directory**:
  ...
  dist
  ```
* **Install Command** (optional – auto):

  ```bash
  npm install
  ```

You don’t need to change `package.json` for Vercel; Vite defaults are fine.

---

## 🔹 Step 5: Set `VITE_API_URL` for production
...
If you already had a `.env` file in `frontend` with a local URL (`http://localhost:8000`), that’s fine.
...
Click **“Deploy”** in the Vercel project.
...
If it succeeds, you’ll see a URL like:
...
* If so → 🎉 you now have a fully deployed, end-to-end working Phase 1 app:

  * Vercel (frontend) → Cloud Run (backend) → Neon (DB)

---

## 🔹 Step 8: CORS (if you get CORS errors)
